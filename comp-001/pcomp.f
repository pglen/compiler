...
\<deny\>                            {
                                ...
                                printf("<DENY> ");
	#endif
                                yylval.strval = strdup(yytext); return(DENY);
                                }
\<yned\>                           {
	#ifdef DEBUGLEX
                                printf("<YNED> ");
	#endif
                                yylval.strval = strdup(yytext); return(YNED);
                                }
       /* --------------------------------------------------------- */
debug                          {
	#ifdef DEBUGLEX
...
\{                              {
	#ifdef DEBUGLEX
                                if(testflex)
    	                            printf("[lcur] '%s' ", (char*)yytext);
	#endif
                                yylval.strval = strdup(yytext); return(LCUR);
                                }

\}                              {
	#ifdef DEBUGLEX
                                if(testflex)
    	                            printf("[rcur] '%s' ", (char*)yytext);
	#endif
                                yylval.strval = strdup(yytext); return(RCUR);
                                }
...

{
   ...
   while (1) {
       ...
       static struct option long_options[] = {
        ...
        c = getopt_long (argc, argv, "abc:d:012fy",
